...
func NewRouter(hmacSecret string, handler *NotificationHandler) http.Handler {
	...
	protected.HandleFunc("/api/notifications/read-all", handler.MarkAllAsRead)
	protected.HandleFunc("/api/notifications/", handler.MarkAsRead) // Matches /api/notifications/{id}/read in go 1.22+
